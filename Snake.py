# Snake and Ladders Simulation Game
# Features:
# - Board design (10x10)
...
# Initialize Pygame's mixer to play sound effects
pygame.mixer.init()
...
# Dictionary for snake start and end positions
SNAKES = {
...
# Dictionary for ladder start and end positions
LADDERS = {
...
class GameGUI(tk.Tk):
    def __init__(self, players):
        ...
        self.dice_img_id.pack(pady=(CELL_HEIGHT * 2.5, 0))  # Vertically centered
        ...
        self.info = tk.Label(
            ctrl, text="", font=("Helvetica", 14, "bold"),
        ...
        self.update_info()  # Show initial player turn
    ...
    # Handle dice rolling animation and movement logic
    def roll_dice(self):
        self.btn.config(state=tk.DISABLED)  # Disable button during roll
        ...
        def animate():
            ...
            self.after(100, lambda: self.after_roll(roll))  # Continue after roll

        threading.Thread(target=animate, daemon=True).start()

    # After dice is rolled, move player and check game logic
    def after_roll(self, roll):
        ...
        # Handle move animation then check for snake/ladder/win
    ...
    def save_score(self, player, duration):
        entry = {"player": player, "time": duration}
        lb = []
        if os.path.exists(LEADERBOARD_FILE):
            lb = json.load(open(LEADERBOARD_FILE))
        lb.append(entry)
        lb = sorted(lb, key=lambda e: e["time"])[:5]  # Top 5 scores
